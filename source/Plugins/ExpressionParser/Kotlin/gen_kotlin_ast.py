...
class Member(object):
    ...
    def __init__(self, name, typename):
        ...
        if self.is_value:
            ...
            self.argtype = 'KotlinAST' + typename[2:]
            self.mtype = 'std::vector<std::unique_ptr<%s> >' % self.argtype
        else:
            self.argtype = 'KotlinAST' + typename
...
def startClass(name, parent, out):
    out.write("""
class KotlinAST%s : public KotlinAST%s
{
  public:
""" % (name, parent))
...
def addConstructor(name, parent, children):
    ...
    childClasses.write('KotlinAST%s(' % name)
    ...
    childClasses.write(') : KotlinAST%s(e%s)' % (parent, name))
    for c in children:
        childClasses.write(', ')
        childClasses.write('%(mname)s(%(sname)s)' % c.__dict__)
    childClasses.write(""" {}
    ~KotlinAST%s() override = default;
""" % name)
...
def addChildren(name, children):
    ...
    case e%(n)s:
        {
            KotlinAST%(n)s *n = llvm::cast<KotlinAST%(n)s>(this);
            (void)n;""" % {'n': name})
    for c in children:
        if c.is_list:
            childClasses.write("""
...
def addParent(name, parent):
  ...
  protected:
    explicit KotlinAST%s(NodeKind kind) : KotlinASTNode(kind) { }
  private:
...
R KotlinAST%s::Visit(V* v) const
{
    switch(GetKind())
    {""" % k
    for subtype in l:
        print """    case e%(n)s:
        return v->Visit%(n)s(llvm::cast<const KotlinAST%(n)s>(this));""" % {'n': subtype}
