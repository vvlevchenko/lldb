...
DWARFASTParserKotlin::DWARFASTParserKotlin(KotlinASTContext &ast) : m_ast(ast) {}

DWARFASTParserKotlin::~DWARFASTParserKotlin() {}

TypeSP DWARFASTParserKotlin::ParseBaseTypeFromDIE(const DWARFDIE &die) {
...
TypeSP DWARFASTParserKotlin::ParseArrayTypeFromDIE(const DWARFDIE &die) {
    SymbolFileDWARF *dwarf = die.GetDWARF();
    dwarf->m_die_to_type[die.GetDIE()] = DIE_IS_BEING_PARSED;
...
TypeSP DWARFASTParserKotlin::ParseReferenceTypeFromDIE(const DWARFDIE &die) {
    SymbolFileDWARF *dwarf = die.GetDWARF();
    dwarf->m_die_to_type[die.GetDIE()] = DIE_IS_BEING_PARSED;
...
lldb::TypeSP DWARFASTParserKotlin::ParseClassTypeFromDIE(const DWARFDIE &die,
                                                       bool &is_new_type) {
    SymbolFileDWARF *dwarf = die.GetDWARF();
    dwarf->m_die_to_type[die.GetDIE()] = DIE_IS_BEING_PARSED;
...
lldb::TypeSP DWARFASTParserKotlin::ParseTypeFromDWARF(
...
lldb_private::Function *DWARFASTParserKotlin::ParseFunctionFromDWARF(
        const lldb_private::SymbolContext &sc, const DWARFDIE &die) {
...
bool DWARFASTParserKotlin::CompleteTypeFromDWARF(
    ...
    switch (die.Tag()) {
        ...
        case DW_TAG_class_type: {
            if (die.GetAttributeValueAsUnsigned(DW_AT_declaration, 0) == 0) {
                if (die.HasChildren())
                    ParseChildMembers(die, Kotlin_type);
                m_ast.CompleteObjectType(Kotlin_type);
                return Kotlin_type.IsValid();
            }
        } break;
        default:
            assert(false && "Not a forward Kotlin type declaration!");
            break;
    }
    return false;
}

void DWARFASTParserKotlin::ParseChildMembers(const DWARFDIE &parent_die,
                                           CompilerType &compiler_type) {
                    ...
                    if (attributes.ExtractFormValueAtIndex(i, form_value)) {
                        switch (attributes.AttributeAtIndex(i)) {
                            ...
                            case DW_AT_data_member_location:
                                ...
                                break;
                            case DW_AT_accessibility:
                                // In Kotlin all base class is public so we can ignore this attribute
                                break;
                            default:
                                assert(false && "Unhandled attribute for DW_TAG_member");
                                break;
